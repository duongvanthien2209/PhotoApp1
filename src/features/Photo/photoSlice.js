...
const photo = createSlice({
  ...
  reducers: {
    addPhoto: (state, action) => {
      ...
      // Không return - nếu return thì phải inmutate state lại
      state.push(action.payload);
    },
    removePhoto: (state, action) => {
      // state đã inmutate => do filter tạo ra một array mới => nên phải return
      const photoId = action.payload;
      // console.log(photoId);
      state = state.filter((photoItem) => photoItem.id !== photoId);
      return state;
    },
    ...
    updatePhoto: (state, action) => {
      ...
      const index = state.findIndex(
        (photoItem) => photoItem.id === newPhoto.id,
      );
      if (index >= 0) {
